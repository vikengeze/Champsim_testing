...
void CACHE::handle_fill()
{
						...
						else
						{
							...
							if (lower_level)
								...
								if (cache_type == IS_STLB) {
									...
									int fctb_found_pos = 0;
									if(iflag == 0){ //changed this
									...
									if(answer.first == -1){
										if(!iflag){ //changed this
											if(fctb_found_pos == -10){
												cout << 'BIKA GIA KAPOIO LOGO ANSWER';
												v2p = va_to_pa(read_cpu, RQ.entry[index].instr_id, RQ.entry[index].full_addr, RQ.entry[index].address, RQ.entry[index].ip, RQ.entry[index].type, iflag, 0); //to proteleutaio argument itan iflag 
												pa = v2p.first;
												...
												if(pa == 0){
													v2p = va_to_pa(read_cpu, RQ.entry[index].instr_id, RQ.entry[index].full_addr, RQ.entry[index].address, RQ.entry[index].ip, RQ.entry[index].type, iflag, 0); //to proteleutaio argument itan iflag 
													pa = v2p.first;
												} //va_to_pa returns pa and cstall
											} 	  //va_to_pa_prefetch returns only pa
										}
										else{
									...
									if(iflag == 0){
										free_indexes = sorted_free_distances();
										stlb_prefetcher_operate(RQ.entry[index].address, RQ.entry[index].ip, 0, RQ.entry[index].type, answer.first, warmup_complete[cpu], free_indexes, RQ.entry[index].instr_id, iflag); //last argument was iflag
	...
	void CACHE::fill_cache(uint32_t set, uint32_t way, PACKET *packet)
	{
		...
		block[set][way].prefetch = (packet->type == PREFETCH) ? 1 : 0; //always 0 for some reason
	...
	int CACHE::prefetch_page(uint64_t ip, uint64_t base_addr, uint64_t pf_addr, int fill_level, int pq_id, int free, int update_free, int free_distance, uint64_t id, int type, int iflag, int lad, int confidence, int irip)
	{
		...
		//va_to_pa_prefetch returns pa if its found else 0
		//if(!free)
