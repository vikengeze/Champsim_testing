...
int O3_CPU::prefetch_code_line(uint64_t pf_v_addr, bool cross_pb)
{
	if(pf_v_addr == 0)
	{
	...
	if (L1I.PQ.occupancy < L1I.PQ.SIZE){
		...
		if(!cross_pb){
			...
			STLB.bpbp[2]++; //  beyond page boundaries instruction cache prefetches

			// search in the STLB for the translation of the prefetch
			set = (uint32_t) ((pf_v_addr>>LOG2_PAGE_SIZE) & ((1 << lg2(STLB.NUM_SET)) - 1));
			for (way=0; way<STLB.NUM_WAY; way++){
				if (STLB.block[set][way].address == pf_v_addr>>LOG2_PAGE_SIZE){
					flag = 1; // found in STLB
					break;
				}
			}   

			// search in the ITLB for the translation of the prefetch
			set = (uint32_t) ((pf_v_addr>>LOG2_PAGE_SIZE) & ((1 << lg2(ITLB.NUM_SET)) - 1));
			for (way=0; way<ITLB.NUM_WAY; way++){
				if (ITLB.block[set][way].address == pf_v_addr>>LOG2_PAGE_SIZE){
					flag = 2; // found in ITLB
					break;
				}
			}

			if(flag){
				// the translation is found in the TLB hierarchy
				STLB.bpbp[3]++; 
				...
				STLB.bpbp[4]++; // different page + TLB miss
				if(BPBP_FILTER){
					// Morrigan as a filter for beyond page boundaries instruction cache prefetches
					pair<int, uint64_t> filter = STLB.PQ.check_queue_vpn(pf_v_addr>>LOG2_PAGE_SIZE);
					if(filter.first != -1){
						STLB.morrigan_filter_hits++; 
						pf_pa = STLB.PQ.entry[filter.first].data;
						flag = 3; // found in the PQ of Morrigan
						penalty = 1;
					}
					else{
						pair<uint64_t, uint64_t> temp = va_to_pa(cpu, 0, pf_v_addr, pf_v_addr>>LOG2_PAGE_SIZE, pf_v_addr, 0, 1, 2);
		...
		pf_packet.event_cycle = current_core_cycle[cpu] + penalty;
		...
		if(cross_pb){
			int skip = 0;
			if(flag == 2){
				// found in ITLB or in the PQ of Morrigan
				skip++;
			}
			else if(flag == 1){
				// found in STLB 
				...
				// found in PQ of Morrigan 
				skip++;
			}
			else{
				// place the beyond page boundaries prefetches into the ITLB and STLB
				set = (uint32_t) ((pf_v_addr>>LOG2_PAGE_SIZE) & ((1 << lg2(ITLB.NUM_SET)) - 1));
				for (way=0; way<ITLB.NUM_WAY; way++){
					if (ITLB.block[set][way].lru == ITLB.NUM_WAY-1) 
						break;
				}   

				if (ITLB.block[set][way].valid == 0)
					ITLB.block[set][way].valid = 1;

				ITLB.block[set][way].dirty = 0;
				ITLB.block[set][way].prefetch = 1;
				ITLB.block[set][way].used = 0;
				ITLB.block[set][way].tag = pf_v_addr>>LOG2_PAGE_SIZE;
				ITLB.block[set][way].address = pf_v_addr>>LOG2_PAGE_SIZE;
				ITLB.block[set][way].full_addr = pf_v_addr;
				ITLB.block[set][way].data = pf_pa;
				ITLB.block[set][way].cpu = cpu;

				for (uint32_t i=0; i<ITLB.NUM_WAY; i++) {
					if (ITLB.block[set][i].lru < ITLB.block[set][way].lru)
						ITLB.block[set][i].lru++;
				}   

				ITLB.block[set][way].lru = 0; // promote to the MRU position

				set = (uint32_t) ((pf_v_addr>>LOG2_PAGE_SIZE) & ((1 << lg2(STLB.NUM_SET)) - 1));
				for (way=0; way<STLB.NUM_WAY; way++){
					if (STLB.block[set][way].lru == STLB.NUM_WAY-1) 
						break;
				}   

				if (STLB.block[set][way].valid == 0)
					STLB.block[set][way].valid = 1;

				STLB.block[set][way].dirty = 0;
				STLB.block[set][way].prefetch = 1;
				STLB.block[set][way].used = 0;
				STLB.block[set][way].tag = pf_v_addr>>LOG2_PAGE_SIZE;
				STLB.block[set][way].address = pf_v_addr>>LOG2_PAGE_SIZE;
				STLB.block[set][way].full_addr = pf_v_addr;
				STLB.block[set][way].data = pf_pa;
				STLB.block[set][way].cpu = cpu;

				for (uint32_t i=0; i<STLB.NUM_WAY; i++) {
					if (STLB.block[set][i].lru < STLB.block[set][way].lru)
						STLB.block[set][i].lru++;
				}   

				STLB.block[set][way].lru = 0; // promote to the MRU position
			}
		}
		return 1;
	}
