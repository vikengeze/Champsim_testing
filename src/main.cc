...
void print_roi_stats(uint32_t cpu, CACHE *cache)
{
	...
	cout << "  real: " << setw(10) << cache->pf_real << "  fills: " << setw(10) << cache->pf_fill << endl;
...
int mmu_cache_prefetch_search(uint32_t cpu, uint64_t vpage, int swap, uint64_t instr_id, uint64_t ip, int type, int iflag){
	...
	mmu_hit[0] = ooo_cpu[cpu].STLB.search_pml4(compare_pml4); //page map lv 4
	mmu_hit[1] = ooo_cpu[cpu].STLB.search_pdp(compare_pdp); // PML4(L4) - PDP(L3) - PD(L2) - PTE(L1). -- intel
	if(LOG2_PAGE_SIZE == 12)  //champsim.h - cpu
		mmu_hit[2] = ooo_cpu[cpu].STLB.search_pd(compare_pd);
